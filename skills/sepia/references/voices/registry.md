...
- **Inputs.** Only what the review already records: rubric observed signals with quoted evidence and advisories (fiction), `Failed:` lines, one per check and direction, and the `Style scan:` line (professional). Nothing is re-read or re-judged for this line.
...
| # | Check or scan hit |
|---|---|
| 1 | a `Failed:` line for check 2 (density) with direction padding, whether or not an omission line is also present |
...
| # | Recorded item |
|---|---|
| a | a `Failed:` line for check 9 (sameness of rhythm) with direction short (lines with long or paragraph do not block) |
