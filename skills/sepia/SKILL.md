...
| **refactor** | Minimal in-place revision preserving structure, voice, and intent. Two-stage: full defect list first, then fix item by item, deepest layer first. Skew replace/delete over insert (measured editor ratio 74/18/8). The `Voice fit:` line is not a defect and is excluded from the fix list. The revised text's **share of content words** must not fall below the source's: machine editing of human prose is marked less by the vocabulary it adds than by the function-word filler it pours in around it (measured on English: lexical density d = −3.10 across three editor models and eight edit types, entropy down, diversity barely moved — `references/style-pass.md` §4, last paragraph; applying the same test to other languages is a Sepia inference). The share is a reader's count, not a tokenizer's: content words are the nouns, verbs, adjectives, and adverbs that carry meaning; function words are articles, prepositions, conjunctions, particles, auxiliaries, and pronouns; count words the way that language counts them (words in English, 詞 in Chinese), and count source and revision the same way — the test compares the two under one procedure, so the procedure's choices cancel. The test is taken over the whole revised passage. When the share fell, look at what was added: function words that carry no structural fix — a restored particle or hedge from §4, a softener, a stacked transition — are reverted or paid for by deleting at least as much filler elsewhere in the same passage; function words that a §1–§2 structural fix requires (the finite verb and its subject that replace a nominalization, the second sentence of a split run-on) stay, and their cost is paid from filler too. The reverted item is therefore never a deeper-layer fix. Grammar and meaning outrank the share: when a required fix adds function words and the passage has no filler left to remove (a bare `Cat sat mat.` needs its articles and preposition), the fix stands and the decrease is reported, never hidden and never bought by deleting content. Holding the share is a Sepia inference from that measurement, not a tested intervention. The last line of every refactor output, on every route, after the revised text, is `Content words: share kept | share fell — reverted <what> | share fell — required by <fix>, no filler to remove`. |
...
- **Deletion beats addition** (74% replace / 18% delete / 8% insert). The only additive fixes are real specificity and the function words a grammar or structural fix requires (refactor row); and no register drift: a rewrite must not come out more promotional than its source, nor with a smaller share of content words (refactor row).
